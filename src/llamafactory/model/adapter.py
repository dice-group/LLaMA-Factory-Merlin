...
def _setup_moelora_tuning(
    ...
    if finetuning_args.use_dora:
        raise ValueError("MoE-LoRA currently does not support DoRA.")
    if finetuning_args.pissa_init:
        raise ValueError("MoE-LoRA currently does not support PiSSA initialization.")
    if model_args.use_unsloth or model_args.use_kt:
        raise ValueError("MoE-LoRA is not compatible with KTransformers or Unsloth.")

    if is_trainable:
        logger.info_rank0("Fine-tuning method: MoE-LoRA")
    ...
    if is_trainable and adapter_to_resume is None:
        ...
        routing_id_count = finetuning_args.moelora_task_num
        language_map = load_language_map(finetuning_args.language_map)
        if language_map is not None:
            language_count = len(language_map)
            if routing_id_count == 1:
                routing_id_count = language_count
                logger.info_rank0(
                    f"MoE-LoRA: inferred `moelora_task_num={routing_id_count}` from language_map size."
                )
            elif routing_id_count < language_count:
                raise ValueError(
                    f"`moelora_task_num` ({routing_id_count}) must be >= number of language ids ({language_count})."
                )

        moelora_config = MoELoraConfig(
            task_type=TaskType.CAUSAL_LM,
            inference_mode=False,
            expert_num=finetuning_args.moelora_num_experts,
            task_num=routing_id_count,
            task_embedding_dim=finetuning_args.moelora_task_embedding_dim,
            gate_top_k=finetuning_args.moelora_sparse_top_k,
            gate_temperature=finetuning_args.moelora_gate_temperature,
            **peft_kwargs,
        )
...
def _setup_vanilla_moelora_tuning(
    config: "PretrainedConfig",
    model: "PreTrainedModel",
    model_args: "ModelArguments",
    finetuning_args: "FinetuningArguments",
    ...
    if finetuning_args.use_dora:
        raise ValueError("Vanilla MoE-LoRA currently does not support DoRA.")
    if finetuning_args.pissa_init:
        raise ValueError("Vanilla MoE-LoRA currently supports standard LoRA initialization only.")
    if model_args.use_unsloth or model_args.use_kt:
        raise ValueError("Vanilla MoE-LoRA is not compatible with KTransformers or Unsloth.")

    if is_trainable:
        logger.info_rank0("Fine-tuning method: Vanilla MoE-LoRA")
    ...
    if is_trainable and adapter_to_resume is None:
        ...
        vanilla_moelora_config = VanillaMoELoraConfig(
            task_type=TaskType.CAUSAL_LM,
            inference_mode=False,
            num_experts=finetuning_args.vanilla_moelora_num_experts,
            top_k=finetuning_args.vanilla_moelora_top_k,
            router_aux_loss_coef=finetuning_args.vanilla_moelora_router_aux_loss_coef,
            router_init_range=finetuning_args.vanilla_moelora_router_init_range,
            **peft_kwargs,
        )
        model = get_peft_model(model, vanilla_moelora_config)
...
def init_adapter(
    ...
    elif finetuning_args.finetuning_type == "mixlora":
        model = _setup_mixlora_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
    ...
    elif finetuning_args.finetuning_type == "vanilla_moelora":
        model = _setup_vanilla_moelora_tuning(
            config, model, model_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32
        )
